...
class HouseRenderer {
    constructor(scene) {
        ...
        // Person selection highlight system
        this.selectedPersonId = null;
        this.personHighlightCircle = null;
    ...
    // Create highlight circle for selected person
    createPersonHighlightCircle() {
        // Remove existing highlight circle if it exists
        this.removePersonHighlightCircle();
        
        // Create a ring geometry for the highlight circle
        const ringGeometry = new THREE.RingGeometry(1.8, 2.3, 32);
        const ringMaterial = new THREE.MeshBasicMaterial({
            color: 0xff0000, // Bright red for better visibility
            transparent: true,
            opacity: 1.0,
            side: THREE.DoubleSide
        });
        
        this.personHighlightCircle = new THREE.Mesh(ringGeometry, ringMaterial);
        this.personHighlightCircle.rotation.x = -Math.PI / 2; // Lay flat on the ground
        this.personHighlightCircle.position.y = 0.2; // Well above ground to be clearly visible
        
        // Store animation properties for pulsing effect
        this.personHighlightCircle.userData.originalOpacity = 1.0;
        this.personHighlightCircle.userData.pulseTime = 0;
        
        // Also create a simple sphere for testing visibility
        ...
        this.personHighlightCircle.userData.testSphere = new THREE.Mesh(sphereGeometry, sphereMaterial);
        this.personHighlightCircle.userData.testSphere.position.y = 1.0; // Above the person
        
        // Add to house group
        if (this.houseGroup) {
            this.houseGroup.add(this.personHighlightCircle);
            this.houseGroup.add(this.personHighlightCircle.userData.testSphere);
            log('DEBUG', 'Added highlight circle and test sphere to houseGroup');
        } else {
            this.scene.add(this.personHighlightCircle);
            this.scene.add(this.personHighlightCircle.userData.testSphere);
            log('DEBUG', 'Added highlight circle and test sphere to scene directly');
        }
        
        log('DEBUG', 'Created person highlight circle with test sphere');
    }
    
    // Remove highlight circle
    removePersonHighlightCircle() {
        if (this.personHighlightCircle) {
            // Remove test sphere if it exists
            if (this.personHighlightCircle.userData.testSphere) {
                if (this.houseGroup) {
                    this.houseGroup.remove(this.personHighlightCircle.userData.testSphere);
                } else {
                    this.scene.remove(this.personHighlightCircle.userData.testSphere);
                }
                this.personHighlightCircle.userData.testSphere.geometry.dispose();
                this.personHighlightCircle.userData.testSphere.material.dispose();
            }
            
            if (this.houseGroup) {
                this.houseGroup.remove(this.personHighlightCircle);
            } else {
                this.scene.remove(this.personHighlightCircle);
            }
            this.personHighlightCircle.geometry.dispose();
            this.personHighlightCircle.material.dispose();
            this.personHighlightCircle = null;
            log('DEBUG', 'Removed person highlight circle and test sphere');
        }
    }
    
    // Update highlight circle position to follow selected person
    updatePersonHighlightCircle() {
        if (!this.personHighlightCircle || !this.selectedPersonId) {
            return;
        }
        
        const selectedPersonMesh = this.personMeshes.get(this.selectedPersonId);
        if (selectedPersonMesh) {
            // Position the highlight circle at the person's position
            this.personHighlightCircle.position.x = selectedPersonMesh.position.x;
            this.personHighlightCircle.position.z = selectedPersonMesh.position.z;
            
            // Position the test sphere as well
            if (this.personHighlightCircle.userData.testSphere) {
                this.personHighlightCircle.userData.testSphere.position.x = selectedPersonMesh.position.x;
                this.personHighlightCircle.userData.testSphere.position.z = selectedPersonMesh.position.z;
            }
            
            // Update pulsing animation
            this.personHighlightCircle.userData.pulseTime += 0.1;
            const pulseValue = Math.sin(this.personHighlightCircle.userData.pulseTime) * 0.3 + 0.7;
            this.personHighlightCircle.material.opacity = this.personHighlightCircle.userData.originalOpacity * pulseValue;
            
            // Debug: Log position occasionally
            if (Math.random() < 0.001) { // 0.1% chance per frame
                log('DEBUG', `Highlight circle position: (${this.personHighlightCircle.position.x.toFixed(2)}, ${this.personHighlightCircle.position.z.toFixed(2)})`);
                log('DEBUG', `Person mesh position: (${selectedPersonMesh.position.x.toFixed(2)}, ${selectedPersonMesh.position.z.toFixed(2)})`);
            }
        } else {
            log('WARN', `Selected person mesh not found for ID: ${this.selectedPersonId}`);
        }
    }
    
    // Set selected person and create/update highlight
    setSelectedPerson(personId) {
        log('DEBUG', `setSelectedPerson called with personId: ${personId}`);
        log('DEBUG', `Current selectedPersonId: ${this.selectedPersonId}`);
        log('DEBUG', `Current personHighlightCircle: ${this.personHighlightCircle ? 'exists' : 'null'}`);
        
        if (this.selectedPersonId === personId) {
            log('DEBUG', 'Person already selected, returning early');
        ...
        if (personId) {
            log('DEBUG', `Setting person ${personId} as selected`);
            // Create highlight circle if it doesn't exist
            if (!this.personHighlightCircle) {
                log('DEBUG', 'Creating new highlight circle');
                this.createPersonHighlightCircle();
            }
            // Update position immediately
            this.updatePersonHighlightCircle();
            log('DEBUG', `Set selected person: ${personId}`);
        } else {
            log('DEBUG', 'Clearing selected person');
            // Remove highlight circle when no person is selected
            this.removePersonHighlightCircle();
            log('DEBUG', 'Cleared selected person');
        }
    }
    ...
    updatePersonMeshPositions() {
        ...
        // Update highlight circle position if a person is selected
        this.updatePersonHighlightCircle();
    }
